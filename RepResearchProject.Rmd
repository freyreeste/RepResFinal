...
total number of NAs and percentaje they represent to asses significance
```{r}
...
Since the data size is to big for handling we will use Caret package to slice the sample data smaller for some statistical models
```{r message=FALSE, warning=FALSE}
library(caret)
```
```{r cache=TRUE}
...
Tornados win in both questions, either fatalities, injuries or economic damages. Flash flood is the second in order for crop damage
...
head(ans7,10)
```
The States with most event types are:
```{r,echo=FALSE}
head(ans8,10)
```
Review criteria
